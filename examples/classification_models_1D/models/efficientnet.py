...
def EfficientNet(
    ...
    if not (weights in {'imagenet', 'audioset', None} or tf.io.gfile.exists(weights)):
        raise ValueError('The `weights` argument should be either '
                         ...
                         '(pre-training on ImageNet or AudioSet), '
                         'or the path to the weights file to be loaded.')
...
def block(
    ...
    """An inverted residual block.
      ...
      Args:
          ...
          filters_out: integer, the number of outputs filters.
    ...
    if id_skip and strides == 1 and filters_in == filters_out:
        if drop_rate > 0:
            x = layers.Dropout(drop_rate, noise_shape=(None, 1, 1), name=name + 'drop')(x)
