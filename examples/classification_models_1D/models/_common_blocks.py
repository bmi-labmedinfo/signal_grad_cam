from classification_models_1D import get_submodules_from_kwargs
...
def slice_tensor(x, start, stop, axis):
    if axis == 2:
        ...
        raise ValueError("Slice axis should be in (1, 2), got {}.".format(axis))


def GroupConv1D(filters,
                kernel_size,
                strides=(1,),
                groups=32,
    ...
    slice_axis = 2 if backend.image_data_format() == 'channels_last' else 1

    def layer(input_tensor):
        inp_ch = int(backend.int_shape(input_tensor)[-1] // groups)  # input grouped channels
        out_ch = int(filters // groups)  # outputs grouped channels
...
def expand_dims(x, channels_axis):
    if channels_axis == 2:
        ...
        raise ValueError("Slice axis should be in (1, 2), got {}.".format(channels_axis))
